...
describe('Actions lesson', () => {
    ...
    it('Type command', () => {
        cy.visit("http://127.0.0.1:5500/actionsLesson.html")
        cy.get([placeholder='type ypur name']).clear().type("Amal")
        cy.get([placeholder='type ypur name']).clear().type("Amal",{delay:1000})  //write slower
        cy.get([name=in_0]).clear().type("amal",{force:true})
